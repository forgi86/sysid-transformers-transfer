...
if __name__ == '__main__':
    ...
    if cfg.log_wandb:
        wandb.init(
            ...
            name="test pwh seed:" + str(cfg.seed),
    ...
    train_ds = NonInfinitePWHDataset(seq_num=100, nx=cfg.nx, nu=cfg.nu, ny=cfg.ny,
    ...
    val_ds = NonInfinitePWHDataset(seq_num=20, nx=cfg.nx, nu=cfg.nu, ny=cfg.ny,
                                   seq_len=cfg.seq_len_ctx + cfg.seq_len_new,
                                   mag_range=cfg.mag_range, phase_range=cfg.phase_range,
    ...
    test_dl = DataLoader(test_ds, batch_size=1, num_workers=1)
    ...
    # TODO Load best model and initial model
    ckpt_path = model_dir / f"{cfg.in_file}.pt"
    checkpoint = torch.load(ckpt_path, map_location=device)
    gptconf = Config(**checkpoint["model_args"])
    model_initial = TSTransformer(gptconf)
    state_dict = checkpoint['model']
    model_initial.load_state_dict(state_dict)
    model_initial.to(device)

    ckpt_path = model_dir / f"{cfg.out_file}.pt"
    checkpoint = torch.load(ckpt_path, map_location=device)
    gptconf = Config(**checkpoint["model_args"])
    model_trained = TSTransformer(gptconf)
    state_dict = checkpoint['model']
    ...
    with torch.no_grad():
        model_initial.eval()
        model_trained.eval()
        for i, (batch_y, batch_u) in enumerate(test_dl):
            if device_type == "cuda":
                batch_y = batch_y.pin_memory().to(device, non_blocking=True)
                batch_u = batch_u.pin_memory().to(device, non_blocking=True)
            ...
            batch_y_sim_initial = model_initial(batch_y_ctx, batch_u_ctx, batch_u_new)
            batch_y_sim_trained = model_trained(batch_y_ctx, batch_u_ctx, batch_u_new)

            loss_initial = torch.nn.functional.mse_loss(batch_y_new, batch_y_sim_initial)
            loss_trained = torch.nn.functional.mse_loss(batch_y_new, batch_y_sim_trained)

            print(loss_initial)
            print(loss_trained)

            plt.plot(batch_y_new[0].cpu())
            plt.plot(batch_y_sim_initial[0].cpu())
            plt.plot(batch_y_sim_trained[0].cpu())
            plt.savefig('test')

            out = np.array([batch_y_new.cpu().numpy().flatten(),
                      batch_y_sim_initial.cpu().numpy().flatten(),
                      batch_y_sim_trained.cpu().numpy().flatten()])

            np.save(model_dir / f"outputs_{cfg.out_file}.npy", out)
